...
const VisitsList = () => {
  ...
  useEffect(() => {
    ...
    setFilteredVisits(visits.filter((visit) => {
      ...
      return (
        (search.toLowerCase() === '' || visit.visitor.toLowerCase().includes(search)) &&
    ...
    <div className="container-fluid side-container">
      <div className="row side-row">
        <header className='flex flex-col md:flex-row justify-between items-center p-2 md:p-2'>
          <h2 className='text-2xl font-bold text-center md:text-left  mb-3 md:mb-0'>Visits</h2>
        ...
        <div className="overflow-x-auto ">
          <Table striped bordered hover className="react-bootstrap-table w-full " id="dataTable">
            ...
            <tbody>
              {filteredVisits.slice(offset, offset + itemsPerPage).map((visit) => (
                <tr key={visit.id}>
                  <td>{visit.id}</td>
                  <td>{visit.visitor}</td>
                  <td>{visit.host}</td>
